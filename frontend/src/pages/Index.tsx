...
const Index = () => {
  ...
  const handleToggleChat = () => {
    ...
    if (chatCollapsed) {
      // Expand back to the last non-collapsed size
      chatPanelRef.current?.expand();
    } else {
      // Collapse to collapsedSize (0)
  ...
  return (
    <div className="h-screen w-full bg-background flex flex-col overflow-hidden">
      {/* Header */}
      <header className="h-16 border-b border-border flex items-center justify-between px-6 bg-panel-darker relative">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-bold rainbow-text">Rainbow</h1>
        </div>

        {/* Collapse/Expand Chat Button - positioned at chat's right edge */}
        <Button
          variant="ghost"
          size="icon"
          onClick={handleToggleChat}
          className={`absolute bg-black text-white hover:bg-muted hover:text-white h-8 w-8 ${
            isAnimating ? "transition-all duration-300" : ""
          }`}
          style={{
            left: chatCollapsed ? "140px" : `calc(${chatWidth}% - 32px)`,
          }}
        >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            <ToggleGroupItem value="create" className="data-[state=on]:bg-muted data-[state=on]:text-foreground px-6">
              Create
            </ToggleGroupItem>
            <ToggleGroupItem value="scenes" className="data-[state=on]:bg-muted data-[state=on]:text-foreground px-6">
              Scenes
            </ToggleGroupItem>
            <ToggleGroupItem value="assets" className="data-[state=on]:bg-muted data-[state=on]:text-foreground px-6">
              Assets
            </ToggleGroupItem>
          </ToggleGroup>
      ...
      <div className="flex-1 overflow-hidden">
        <ResizablePanelGroup direction="horizontal" className="flex-1">
          ...
          <ResizablePanel
            ...
            onResize={(size) => {
              // Track current width so the toggle button sits at the panel edge
              setChatWidth(size);
            }}
          >
            <div
          ...
          {!chatCollapsed && <ResizableHandle withHandle />}

          {/* Right Side - Content Area */}
          <ResizablePanel defaultSize={75} className={isAnimating ? "transition-all duration-300" : ""}>
            {mode === "create" ? (
